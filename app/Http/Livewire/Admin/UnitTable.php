...
class UnitTable extends Component
{
    ...
    public function render()
    {
        ...
        $unit = Unit::orderBy($this->sortField, $this->sortAsc ? 'asc' : 'desc');
        if (!empty($search)) $unit = $unit->where(function ($q) use ($search) {
            $q->where('nama_unit', 'LIKE', '%' . $search . '%')
                ->orWhere('singkatan', 'LIKE', '%' . $search . '%');
        });

        return view('livewire.admin.unit-table', [
            'unit' => $unit->paginate($this->perPage),
        ]);
    }
    ...
    public function delete()
    {
        ...
        Unit::find($this->id_delete)->delete();
        $this->id_delete = null;
